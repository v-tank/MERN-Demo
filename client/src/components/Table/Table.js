...
class EnhancedTable extends React.Component {
  ...
  // function that opens form dialog to add location
  handleClickOpen = () => {
  ...
  handleSubmit = () => {
    ...
    if (this.state.streetAddr === '' || this.state.city === '' || this.state.state === '' || this.state.stateAbbr === '' || this.state.zipcode === '' || this.state.lat === '' || this.state.long === '') {
      ...
      axios.post('/api/location/', obj).then(res => {
  ...
  handleDelete = () => {
    ...
    this.state.selected.forEach(el => {
      ...
      // console.log('/api/location/' + entryToDelete);

      axios.delete('/api/location/' + entryToDelete).then(res => {
        var queryURL = '/api/location';
        axios.get(queryURL).then(response => {
          this.props.updateStateAfterFetch(response.data);
        });
